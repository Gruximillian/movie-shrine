...
const ShowUserListMedia = (props) => {
  ...
  const { mediaType, userDetails } = props;
  ...
  const noResultsMessage = () => (
    <div className={classes.NoResultsMessage}>You have no items in your {mediaMessage}</div>
  );

  const loadMoreButton = () => {
    if (mediaList && mediaList.length > itemsToShow.length)
      return (
  ...
  return (
    <main className={classes.Main}>
      <p className={classes.WelcomeMessage}>Your {mediaMessage}:</p>
