...
const TmdbActions = (props) => {
  const {
    mediaItem,
    userDetails: { favorite, watchlist, id },
  ...
  const favouriteClass = `${classes.TmdbAction} ${classes.TmdbActionAddToFavourite} ${
    isFavourite ? classes.isFavourite : ''
  }`;
  const watchlistClass = `${classes.TmdbAction} ${classes.TmdbActionAddToWatchlist} ${
    isInWatchList ? classes.isInWatchlist : ''
  }`;

  return (
    <div data-preventclick className={classes.TmdbActionsContainer}>
      <div className={favouriteClass} data-preventclick onClick={() => toggleItemInList('favorite')}>
        {icons.star}
      </div>
      <div className={watchlistClass} data-preventclick onClick={() => toggleItemInList('watchlist')}>
        {icons.watch}
      </div>
    </div>
  );
...
const mapDispatchToProps = (dispatch) => {
  return {
    initiateToggleItemInMediaList: (listType, mediaItem, userId, isInList) =>
      dispatch(actions.initiateToggleItemInMediaList(listType, mediaItem, userId, isInList))
  };
};
