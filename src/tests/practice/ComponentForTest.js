...
const ComponentForTest = (props) => {
  ...
  return (
    <div data-test="component-component-for-test">
      ...
      <button data-test="increment-button" onClick={() => updateCountBy(1)}>
        Increase counter
      </button>
      <button data-test="decrement-button" onClick={() => updateCountBy(-1)}>
        Increase counter
      </button>
      {errorMessage && <p data-test="error-message">{errorMessage}</p>}
    </div>
  );
};
